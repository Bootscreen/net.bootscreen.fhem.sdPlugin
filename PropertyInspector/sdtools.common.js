...
function connectElgatoStreamDeckSocket(inPort, inUUID, inRegisterEvent, inInfo, inActionInfo) {
    ...
    console.log("inActionInfo", inActionInfo);
...
function setSettings(global = false) {
    var payload = {};
    if (global) {
        var elements = document.getElementsByClassName("sdGlobalProperty");
    }
    else {
    ...
    setSettingsToPlugin(payload, global);
}

function setSettingsToPlugin(payload, global = false) {
    var settings = global ? 'setGlobalSettings' : 'setSettings';
...
function getSettingsFromPlugin(global = false) {
    var settings = global ? 'getGlobalSettings' : 'getSettings';
    if (websocket && (websocket.readyState === 1)) {
        const json = {
            'event': settings,
            'context': uuid
